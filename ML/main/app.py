...
namespace = api.namespace('recommend', description='레시피 추천 API')
...
if df_likes.empty:
    ...
    def predict_likes_topsim(likes_arr, recipes_arr, N):
        # 특정 아이템과 유사도가 높은 상위 N개 (20개 넘으면 20개까지만)
        if N > 20:
            N = 20
    ...
    # userId 9가 좋아요한 레시피
    user_likes_id = likes_matrix.loc[9, :]
    var = user_likes_id[user_likes_id > 0].sort_values(ascending=False)[:]
    print(var)

    # 아직 좋아요를 누르지 않은 레시피 리스트 구하기
    def get_not_like_recipes(likes_matrix, userId):

        # userId의 좋아요 정보
        user_likes = likes_matrix.loc[userId,:]
    ...
    def recomm_recipe_by_userid(pred_df, userId, not_like_list, N):
        recomm_recipes = pred_df.loc[userId, not_like_list].sort_values(ascending=False)[:N]

        return recomm_recipes

    # 아직 좋아요를 누르지 않은 레시피 리스트
    not_like_list = get_not_like_recipes(likes_matrix, 9)

    # 아이템 기반의 최근접 이웃 협업 필터링으로 레시피 추천
    recomm_recipes = recomm_recipe_by_userid(likes_pred_matrix, 9, not_like_list, 1)
